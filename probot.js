...
if(message.content.startsWith(prefix  +  'warn'))  {
  ...
  if(!user)  return  message.channel.send("**  -  mention  a  member  **")//by  Rodeo
  if(!reason)  return  message.channel.send("**  -  Type  Reason  **")//by  Rodeo
  let  reportembed  =  new  Discord.RichEmbed()
  .setTitle(`**New  Warned User !**`)
.addField("**-  Warned  User:**",  `[${user}  with  ID  ${user.id}]`)//by  Rodeo
.addField('**-  Warned  By:**',`[${message.author.tag} with id ${message.author.id}]`)//by  Rodeo
...
client.on('message', async message => {
    ...
    let args = message.content.split(' ').slice(1);
if (message.content.startsWith("²detecolors")) {
  if(!message.member.hasPermission('ADMINISTRATOR')) return
